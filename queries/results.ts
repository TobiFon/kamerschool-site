...
//===========================================================================
// BULK REPORT CARD JOB TYPES
//===========================================================================

export interface BulkReportCardJob {
  id: number;
...
// This represents the structure of the final JSON file generated by the backend
export interface BulkResultsData {
  job_id: number;
  school_info: School; // Assuming a School type exists
  students: StudentDetailedResultsResponse[]; // The detailed results for each student
}

//===========================================================================
// BULK REPORT CARD JOB QUERIES
//===========================================================================

export async function createBulkReportJob(
  payload: BulkJobPayload
): Promise<BulkReportCardJob> {
  const res = await authFetch(
    `${process.env.NEXT_PUBLIC_API_URL}/results/bulk-report-jobs/`,
    {
      method: "POST",
  ...
  if (!res.ok) {
    ...
    throw new Error(
      Object.values(errorData).flat().join(", ") ||
        "Failed to start bulk report generation job."
    );
  }
  return res.json();
}

export async function fetchBulkReportJobs(): Promise<BulkReportCardJob[]> {
  const res = await authFetch(
    `${process.env.NEXT_PUBLIC_API_URL}/results/bulk-report-jobs/`
  );
  if (!res.ok) {
    throw new Error("Failed to fetch bulk report jobs.");
  }

  const data = await res.json();

  // THIS IS THE CRITICAL FIX:
  // This line gracefully handles both paginated and non-paginated responses.
  // If the API returns { "results": [...] }, it will correctly extract the array.
  // If the API (after the backend fix) returns just [...], it will also work.
  return data.results || data;
}

export async function fetchBulkReportJobById(
  jobId: number
): Promise<BulkReportCardJob> {
  const res = await authFetch(
    `${process.env.NEXT_PUBLIC_API_URL}/results/bulk-report-jobs/${jobId}/`
  );
  if (!res.ok) {
...
// This function fetches the final JSON data file, not job metadata
export async function fetchBulkResultsData(
  url: string
): Promise<BulkResultsData> {
  // We use a simple fetch here because the URL is a direct link to a media file
  // and may not require the same auth headers, depending on your storage setup.
  // If your media files are protected, you might need to use authFetch.
  const res = await fetch(url);
  if (!res.ok) {
    throw new Error("Failed to download the report data file.");
  }
  return res.json();
}
